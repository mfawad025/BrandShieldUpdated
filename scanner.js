...
async function attemptTakedownNotice(leak, user) {
  ...
  const abuseEmail = guessAbuseEmail(leak.url);
  if (!mailer || !abuseEmail) return false;
  ...
  const body = [
    ...
    ``,
    `Reference: BrandShield case for ${user.email}`,
  ].join('\n');
...
module.exports = { runDailyScanForUser, runDailyScanForAllUsers };
