...
function initFreeScan() {
  ...
  async function runScan() {
    ...
    if (alias.length < 2) {
      ...
      resultsBox.innerHTML = `<p style="color:#F5586B; font-size:.85rem; margin:0;">Enter a name or username to scan.</p>`;
      return;
    }
    ...
    try {
      const res = await fetch(`${BRANDSHIELD_BACKEND_URL}/api/scan/free`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
      ...
      if (!res.ok) {
        resultsBox.innerHTML = `<p style="color:#F5586B; font-size:.85rem; margin:0;">${escapeHtml(data.error || 'Scan failed — please try again shortly.')}</p>`;
        return;
      }
      ...
      resultsBox.innerHTML = `<p style="color:#F5586B; font-size:.85rem; margin:0;">Could not reach the scan server. Please try again in a moment.</p>`;
    } finally {
      btn.disabled = false;
